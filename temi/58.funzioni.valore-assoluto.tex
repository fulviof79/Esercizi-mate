...
\exonly{
	...
	\begin{tikzpicture}[baseline={($(current bounding box.north)-(0,1.6ex)$)}]
		...
		width=12cm,
			...
			\addplot [only marks, mark=o] (-1/3,0) node[below] {$A$} ;
			\addplot [only marks, mark=o] (3,0) node[above right] {$B$} ;
...
\begin{parts}
	\part 
	\exonly{Quale vincolo tra $f$ e $g$ ha come soluzione l'intervallo  $[A;B]$ ?}
