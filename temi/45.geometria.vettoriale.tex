...
\begin{questions}
	...
	\begin{qblock}
		...
		\begin{parts}
			...
			\solonly{
				$\vec{a}=\vecII{3}{2}$, $\vec{b}=\vecII{-1}{2}$, $\vec{c}=\vecII{0}{-2}$
			}
			\part
			\exonly{rispetto alla standard}
			\solonly{
				$\vec{a}=\vecII{8}{5}$, $\vec{b}=\vecII{0}{9}$, $\vec{c}=\vecII{-2}{-8}$
			}
		\end{parts}
	\end{qblock}
	...
	\begin{qblock}
		\question
		\exonly{Dati i vettori $\vec{a}=\vecII{3}{5}$, $\vec{b}=\vecII{-4}{2}$ e $\vec{c}=\vecII{-3}{-4}$ determinare le componenti di $\vec{d}$, $\vec{e}$, $\vec{f}$ e $\vec{g}$ definiti come segue:


			\begin{multicols}{2}
				$\vec{d}=\vec{a}+\vec{b}+\vec{c}$\\
				$\vec{e}=\vec{a}-\vec{b}-\vec{c}$ \\
				$\vec{f}=\frac{1}{2}\vec{a}$ \\
				$\vec{g}=-\dfrac{3}{2}\vec{c}$
			\end{multicols}
	...
	\begin{qblock}
		...
		\question
		\exonly{Dati i punti $A(2;3)$, $B(-1;2)$ e $C(5;-4)$ determinare le componenti dei vettori $\overrightarrow{AB}$,$\overrightarrow{BC}$ e $\overrightarrow{AC}$}
...
\begin{questions}
	\begin{qblock}
		\question
		\exonly{Dati i vettori $\vec{a}=\vvec{-2,3,1}$ e  $\vec{b}=\vvec{4,5,-1}$ determinare:
		}
		\begin{parts}
			\part \exonly{Le norme di $\vec{a}$ e $\vec{b}$}
			\solonly{$\norm{\vec{a}}=\sqrt{14}$ e $\norm{\vec{b}}=\sqrt{42}$}
			\part \exonly{L'angolo tra i due vettori}
			\solonly{$\num{75.67}\degree$}
			\part 
			\exonly{La lunghezza della proiezione di $\vec{a}$ su $\vec{b}$}
			\solonly{$\dfrac{6}{\sqrt{42}}\approx \num{0.93}$}
		\end{parts}
...
\begin{questions}
	...
	\begin{qblock}
		\question
		\exonly{Dati i punti  $A(-2; -1;4)$, $B(6; 1;-2)$, $C(5; 5;1)$ e $D(\frac{3}{2};4;4)$}
		%\solonly{\hfill}

		\begin{parts}

			\part
			\exonly{Determinare se $ABCD$ è un quadrilatero. Ovvero se i punti $A$, $B$, $C$ e $D$ giacciono sullo stesso piano.}
			\solonly{
				No, i punti non sono complanari. 
			}
	

			\part 
			\exonly{Determinare il punto di intersezione delle diagonali del quadrilatero $ABCD_1$ con $D_1(1;4;4)$}
			...
			\part
			\exonly{Determinare la distanza tra il punto d’intersezione e la retta passante per $A$ e $D_1$}
			\solonly{$\approx \num{2.79}$}

			\part

			\exonly{Determinare l’area del quadrilatero $ABCD_1$}
			\solonly{$\num{36.59}$}
		\end{parts}
	\end{qblock}
	...
	\begin{qblock}
		...
		\exonly{
			...
			\nopagebreak
			$r_1:\vvec{x,y,z}=\vvec{-2,2,3}+k_1\vvec{1,-1,2}$

			\nopagebreak
			$r_2:\vvec{x,y,z}=\vvec{1,3,1}+k_2\vvec{-1,3,4}$

		}

		\nopagebreak
		\solonly{$\dfrac{4\sqrt{35}}{7}\approx 3.38$}
