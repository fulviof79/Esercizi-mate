...
\question
\exonly{
Consideriamo le forze $\vec{F_1}$ e $\vec{F_2}$. Determinare il vettore della forza $\vec{F_3}$ tale per cui $\vec{F_1}+\vec{F_2}+\vec{F_3}=\vec{0}$.}
...
\question
\exonly{
Consideriamo le forze $\vec{F_1}$ e $\vec{F_2}$. Determinare il vettore della forza $\vec{F_3}$ tale per cui $\vec{F_1}+\vec{F_2}+\vec{F_3}=\vec{R}$.}
...
\exonly{Dati i vettori $\vec{a}=\vecII{2}{3}$, $\vec{b}=\vecII{-4}{2}$.
 ...
 Calcolare $\norm{\vec{a}}$, $\norm{\vec{b}}$ e $\norm{\vec{a}+\vec{b}}$}
\solonly{$\norm{\vec{a}}=\sqrt{13}$, $\norm{\vec{b}}=\sqrt{20}$ e $\norm{\vec{a}+\vec{b}}=\sqrt{29}$}

\exnewpage
\question 
\exonly{
Dari i vettori $\vec{v}=\vecII{3}{-5}$e $\vec{w}=\vecII{-2}{3}$ calcolare:
}
%\solonly{\hfill}
\begin{multicols}{2}
\begin{parts}
...
\question
\exonly{Calcolare le componenti (approssimate a due decimali) dei vettori $\vec{a}$ e $\vec{b}$ di norma $2$

\begin{tikzpicture}[baseline={($(current bounding box.north)-(0,1.6ex)$)}]
\draw[-latex] (-3,0) -- (3,0) coordinate (X);
\draw[-latex] (0,-3) -- (0,3);
\draw[->] (0,0) coordinate (O)-- node[midway,above]{$\vec{a}$} +(1,3) coordinate (A);
...
\question
\exonly{
Un aeroplano vola in direzione nord ad una velocità di
 \SI{700}{\kilo\meter/\hour}. Un vento di \SI{120}{\kilo\meter/\hour}
 proveniente da est ne perturba la traiettoria. 
Calcolare intensità e direzione della velocità dell'aeroplano.
}
...
Trapezio: $ \overrightarrow{AD} \parallel \overrightarrow{BC}$}
\end{parts}
\end{questions}
%\exonly{\newpage}
\newpage
...
%\solcbreak
\question
\exonly{Dati i punti $A(-2;3)$, $B(1;-1)$ e $C(3;\sfrac{1}{2})$. Determinare le coordinate di $D$ tali per cui il quadrilatero $ABCD$ sia un rettangolo.}
...
\solonly{$\sqrt{22}$, $\sqrt{30}$ e $\sqrt{6}$}
...
\part
\exonly{Determinare le coordinate di $H$, $G$ e $F$ tali per cui il solido risultante (vedi schizzo) sia un parallelepipedo}
...
\part
\exonly{Ammettendo che l'unità sia di \SI{1}{\centi\metre} calcolare la superficie di $ABCD$}
\solonly{$\num{27.75} \si{\square\centi\metre}$}

\part
\exonly{Determinare le coordinate del punto $I$ sul segmento [AE] tale per cui a distanza tra $A$ e $I$ sia di $2$ unità.}
\solonly{$I(\num{0.24},-\num{0.38},\num{0.44})$}
\end{parts}
\end{questions}
...
\begin{questions}
	...
	\begin{parts}
		...
		\part
		\exonly{Determinare la distanza tra punto d’intersezione e la retta passante per A e D}
		\solonly{$\approx \num{2.9}$}
