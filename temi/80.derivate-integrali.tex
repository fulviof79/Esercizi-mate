...
\begin{questions}
	...
	\begin{qblock}
		...
		\exonly{
			...
			\begin{parts}
				\part
				\exonly{ Determinarne la derivata $f'(x)$}
				\solonly{$f'(x)=4x-3$}
				\part 
				\exonly{ Rappresentare graficamente $f(x)$ e $f'(x)$ sullo stesso diagramma.}
				\solonly{ 
                    ...
                    \begin{axis}[
                        ...
                        \addplot[draw=red]{2*x^2-3*x-5} node[pos=0.8,above] {$f(x)$};
                        \addplot[draw=blue]{4*x-3}node[pos=0.8,above] {$f'(x)$};
                    \end{axis}
                \end{tikzpicture}
				}

				\part 
					\exonly{ Determinare le coordinate del vertice di $f(x)$.}
					\solonly{ $V(\dfrac{3}{4};-\dfrac{49}{8})$ }

				\part 
				\exonly{ Determinare la pendenza di $f(x)$ nel punto di ascissa $x=2$.}
				\solonly{ $f'(2)=5$ }

				\part 
				\exonly{ Determinare la pendenza di $f(x)$ nel punto di ascissa $x=-1$.}
				\solonly{ $f'(-1)=-7$ }

				\part 
				\exonly{ Determinare in che punti $f(x)$ ha una pendenza di $-1$.}
				\solonly{ $x=\dfrac{1}{2}$ }
			\end{parts}
	...
	\begin{qblock}
		...
		\exonly{
			...
			\begin{parts}
				\part 
				\exonly{Determinare i punti di intersezione tra $f(x)$ e $g(x)$.}
				\solonly{ $I_1\left( \dfrac{5+\sqrt{65}}{10} ;  \dfrac{17 -\sqrt{65}}{10}  \right) $
				$ I_2\left( \dfrac{5-\sqrt{65}}{10} ;  \dfrac{17 +\sqrt{65}}{10}  \right) $  }
				 
				\part 
				\exonly{Determinare la pendenza di $f(x)$ e di $g(x)$ nel punto di ascissa $x=-4$.}
				\solonly{ $f'(-4)=-\dfrac{11}{2}$ , $g'(-4)=17$ }

				\part 
				\exonly{Determinare in che punti $f(x)$ e $g(x)$ hanno pendenza $1$.}
				\solonly{ $f$ in $x=\dfrac{5}{2}$ , $g$ in $x=0$} 

				\part 
				\exonly{Determinare in che punti $f(x)$ e $g(x)$ hanno la stessa pendenza.}
				\solonly{ $x=\dfrac{1}{2}$ }
	...
	\begin{qblock}
		...
		\begin{parts}
			\part 
			\exonly{Determinarne le intersezioni con gli assi. }
			\solonly{$I_x=\left\lbrace (-\frac{1}{3};0),(2;0)\right\rbrace $ $I_y(0;-2)$ }

			\part 
			\exonly{Determinarne gli estremi locali. }
			\solonly{Minimo $\left( \frac{5}{6};-\frac{49}{12}\right) $ }

			\part 
			\exonly{Tracciare uno schizzo del grafico utilizzando i dati trovati. }
	...
	\begin{qblock}
		...
		\begin{parts}
			\part 
			\exonly{Determinarne le intersezioni con gli assi. }
			\solonly{$I_x=\left\lbrace (\dfrac{5 + \sqrt{41}}{4};0),(0;0),(\dfrac{5 - \sqrt{41}}{4};0)\right\rbrace $ $I_y(0;0)$ }

			\part 
			\exonly{Determinarne gli estremi locali. }
			\solonly{Max $\left( \frac{5-\sqrt{37}}{6};0.186\right) $  Min $\left( \frac{5+\sqrt{37}}{6};-8.149 \right) $ }
	...
	\begin{qblock}
		...
		\begin{parts}
			...
			\solonly{
				Min $\in \left\lbrace \left( -\frac{\sqrt{6}}{2};-\frac{9}{4}\right), \left( \frac{\sqrt{6}}{2};-\frac{9}{4}\right) \right\rbrace $ Max$\left( 0;0\right) $ }
...
\begin{questions}
	...
	\begin{qblock}
		...
		\solonly{
			\begin{parts}
				\part $x_1 \approx -0.9$ , $x_2 \approx 0.45$ , $x_3 \approx 2.45$
				\part $x \approx 1.43$
				\part $x_1 \approx -1.91$ , $x_2 \approx 1.	66$
			\end{parts}
		}
	\end{qblock}
\end{questions}
...
\begin{questions}
	...
	\begin{qblock}
		\question
		La curva $c:y=\cos(x)$ viene approssimata utilizzando la curva $d:y=1-\frac{x^2}{2}$.

		Determinare (con 4 cifre significative):

		\begin{parts}
			\part l'area della superficie compresa tra le due curve nell'intervallo $[0;\frac{\pi}{4}]$,
			...
			\part l'area della superficie compresa tra la curva $d$ e l'asse delle ascisse nello stesso intervallo.
